...
def start():
    ...
    while True:
        ...
        if answer == '1':
            date = model.see_all(d)                             #показывает весь список
            view.show_contacts(d)
        elif answer == '2':
            phone, name = input('Введите номер телефона и через пробел имя, фамилию с разделением через нижнее подчеркивание: ').split()
            res = model.add_contact(name, phone)                     # добавить контакт
            view.result(res)                        #выводит результат успешно или нет
        elif answer == '3':
            name = input('Введите имя контакта для поиска: ')    #поиск контакта по любой характеристике
            result = model.find_name(name)                  
            view.show_contacts(result)
        elif answer == '4':
            phone, name = input('Введите номер и имя удаляемого контакта: ')    #удаление контакта по любым данным
            result = model.delete_name(name)
            view.delete_contacts(result)          #должен выдавать сообщение об удалении контакта
        elif answer == '5':
            ...
            result = model.change_name(name, new_name)    # тут вводится новое значение
